...
def test_months_with_28_or_29():
    ...
    for item in MONTHS_WITH_28_or_29:
        assert days_in_month(item) == 28 and 29

def test_months_with_no_caps():
    """
    Test months which are not capitalized properly.
    """
    for item in MONTHS_WITH_28_or_29 or MONTHS_WITH_30:
        try:
            assert days_in_month(item) == item[0].lower()
        except TypeError:
            return True
